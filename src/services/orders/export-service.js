...
// Get export history (only SPX order exports — exclude invoice exports)
...
// ============================================
// INVOICE EXPORT (HDDT) FUNCTIONS
// ============================================

/**
 * Save an invoice export to R2 and database.
 * @param {object} data - { fileName, fileData (base64), orderIds, orderCount, invoiceRowCount }
 * @param {object} env - Cloudflare environment bindings
 * @returns {object} { success, exportId, fileName }
 */
export async function saveInvoiceExport(data, env) {
    const { fileName, fileData, orderIds, orderCount, invoiceRowCount } = data;

    if (!fileName || !fileData || !orderIds) {
        throw new Error('Missing required fields for invoice export');
    }
    ...
    // Generate file path
    const timestamp = Date.now();
    const filePath = `exports/invoice/${timestamp}_${fileName}`;
    ...
    // Save to database with type='invoice'
    const result = await env.DB.prepare(`
        INSERT INTO export_history (created_at, order_count, order_ids, file_name, file_path, status, type, invoice_row_count)
    ...
    if (!result.success) {
        throw new Error(result.error || 'Failed to save invoice export to database');
    }
...
/**
 * Get invoice export history (type = 'invoice').
 * @param {object} env
 * @returns {object} { success, exports }
 */
export async function getInvoiceExportHistory(env) {
    const { results: exports } = await env.DB.prepare(`
        SELECT
            id,
            created_at,
...
/**
 * Merge multiple invoice exports into one workbook data.
 * @param {number[]} exportIds
 * @param {object} env
 * @returns {object} { success, orders, totalOrders, exportCount }
 */
export async function mergeInvoiceExports(exportIds, env) {
    if (!exportIds || !Array.isArray(exportIds) || exportIds.length === 0) {
        throw new Error('Export IDs array is required');
    }
    ...
    const { results: exports } = await env.DB.prepare(`
        SELECT id, order_ids, file_name
        FROM export_history
        WHERE id IN (${placeholders}) AND type = 'invoice'
    `).bind(...exportIds).all();

    if (!exports || exports.length === 0) {
        throw new Error('No invoice exports found');
    }
    ...
    if (!orders || orders.length === 0) {
        throw new Error('No orders found for invoice merge');
    }
...
export async function markInvoiceExportDownloaded(exportId, env) {
    ...
    // Get order_ids from the export record
    const exportInfo = await env.DB.prepare(`
        SELECT order_ids FROM export_history WHERE id = ? AND type = 'invoice'
    `).bind(exportId).first();

    if (!exportInfo) {
        throw new Error('Invoice export not found');
    }

    const orderIds = JSON.parse(exportInfo.order_ids);
    const now = Date.now();

    // Update export_history status
    await env.DB.prepare(`
        UPDATE export_history
        SET status = 'downloaded', downloaded_at = ?, updated_at = ?
