/**
 * ShopVD — Chọn nguyên liệu → tự điền giá vốn (SP tùy chỉnh trên extension).
 * Module độc lập: cache API, modal gọn, event delegation.
 */
(function (global) {
  'use strict';
  ...
  function apply() {
    ...
    const total = calcTotal(active.selected);
    active.costInput.value = String(total);
    active.costInput.dispatchEvent(new Event('input', { bubbles: true }));
  ...
  async function open(opts) {
    ...
    // Gỡ ô tìm NL cũ nếu còn sót từ bản trước
    ...
    try {
      ...
      console.error('[ShopvdMaterialsCostPicker]', err);
  ...
  global.ShopvdMaterialsCostPicker = {
    open,
    close,
    /** Xóa cache / lựa chọn phiên (khi cần) */
    resetSession() {
      lastSelection = [];
