...
async function runMigration() {
    try {
        console.log('🚀 Starting migration 039: Remove old datetime columns from discounts\n');
        ...
        if (!hasCreatedAtUnix || !hasUpdatedAtUnix) {
            console.error('❌ Error: Unix timestamp columns not found! Run migration 038 first.');
            process.exit(1);
        }
        ...
        await client.execute(`CREATE TABLE discounts_backup_${timestamp.replace(/-/g, '')} AS SELECT * FROM discounts`);
        console.log(`  ✅ Backup created: discounts_backup_${timestamp.replace(/-/g, '')}\n`);

        // Step 3: Drop indexes
        console.log('Step 3: Dropping old indexes...');
        try {
            await client.execute('DROP INDEX IF EXISTS idx_discounts_created_at');
            console.log('  ✅ Dropped idx_discounts_created_at');
        } catch (e) {
            console.log('  ⚠️  idx_discounts_created_at not found');
        }
        
        try {
            await client.execute('DROP INDEX IF EXISTS idx_discounts_updated_at');
            console.log('  ✅ Dropped idx_discounts_updated_at');
        } catch (e) {
            console.log('  ⚠️  idx_discounts_updated_at not found');
        }
        console.log();
        ...
        if (hasCreatedAt) {
            await client.execute('ALTER TABLE discounts DROP COLUMN created_at');
            console.log('  ✅ Dropped created_at column');
        }
        
        if (hasUpdatedAt) {
            await client.execute('ALTER TABLE discounts DROP COLUMN updated_at');
        ...
        const finalTableInfo = await client.execute('PRAGMA table_info(discounts)');
