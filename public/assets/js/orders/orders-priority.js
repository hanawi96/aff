...
/**
 * Update priority icon appearance (both in STT column and Actions column)
 */
function updatePriorityIcon(orderId, isPriority) {
    // Update icon in STT column (existing logic)
    const iconButton = document.querySelector(`[data-priority-order-id="${orderId}"]`);
    
    if (isPriority) {
        // Priority: Show yellow star icon
        if (iconButton) {
            // Icon exists, just ensure it's visible
        ...
        // Normal: Remove icon completely
        if (iconButton) {
            iconButton.remove();
        }
    }
    
    // Update icon in Actions column (new logic)
    const row = document.querySelector(`tr[data-order-id="${orderId}"]`);
    if (row) {
